...
### D-3. 필요도 "상" 인데 아직 못 푼 것 6건

이 여섯 개는 웹 이식에 직접 걸린다 (`UNRESOLVED.md` ①).

| # | 무엇 | 주소 | 왜 문제인가 |
|---|---|---|---|
| U-01 | 페어/파울 판정의 문턱값 | `0x9d660` | 타구 결과에 그대로 영향 |
| U-02 | 2아웃 득점 보류 처리 | 메시지 `0x13` 처리처 | 2아웃에 타자주자가 먼저 아웃되면 득점 무효 |
| U-03 | **승·패·세이브 투수 판정** | `state+0x44 / +0x50 / +0x5c` 를 채우는 곳 | 투수 성적 전체가 걸려 있다 |
| U-04 | 구장 아이템 구매 처리·가격 출처 | `0xd44c4`, `rec+0x1b8/+0x1b9` | 시즌 구단관리에서 통째로 빈 수치 덩어리 |
| U-05 | 드래고나(해금 id 3) 오픈 조건 | `0x2bb48` | 🌐 **로컬로 여는 길이 없다(확정)** — 아래 참조 |
| U-06 | 평판 평가 기록 16칸(`SR+0x1a0`) 의 이름 | 지우는 곳 = 시즌 상태 `0xdd` | 시즌 평판이 무엇으로 오르내리는지의 절반 |
