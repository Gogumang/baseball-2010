...
| **견제 — CPU 도 건다** | 투수 AI 목표점 종류 **4**(`0x345fc` 점프표 `0xcfd94`[4] = `0x34848`)가 견제다. 목표점을 안 만들고 `rand(1,4)` 를 주자 있는 루가 나올 때까지 다시 굴려 **메시지 0x10** 을 보내고 **공을 안 던진다**. `pitchPatterns` 다섯째 가중치 w4 = 주자 있으면 3 · 없으면 0 → **주자가 있는 투구의 3%** | I 4a-2 · S8 7 |
| **견제 — 세이프 콜 17 은 종류 4·5 면 무조건** | 결과 코드 9 의 처리기 `0x51c14` 머리 세 줄이 견제(4)·주자만(5)을 따로 빼서 **다른 검사를 전부 건너뛰고** 소리 **17("Safe!")** 로 간다.<br>`51c14 ldr r3,[[r4]+0x118]` / `51c1e cmp #4 → beq 51c38` / `51c22 cmp #5 → beq 51c38` / `51c38 movs r1,#0x11 → 0x51b18` | S8 6-5 · L 판정 스위치 |
| **견제 — 전용 아웃 식이 없다** | `0xb36d0` 머리의 거르개 `0xb3970 = 0x58d` = 비트 {0,2,3,7,8,10} 에 **4 가 없어** 보통 아웃 판정이 그대로 돈다. 그런데 견제가 걸리면 `0x4677a` 가 살아 있는 주자를 모두 `+0x8c`(마지막으로 닿은 루)로 되돌리고, 태그·포스 두 갈래가 모두 "아직 움직이는 중" 을 요구한다 → **루에 붙은 주자는 견제로 절대 안 죽고, 도루로 루를 떠난 주자만 견제사한다** | I 3c · I 4a-1 · S8 6-5 |
| ⚠️ **견제를 배선할 때** | 원본 `0xa8024` 의 공통 꼬리 `0xa8ac6` 이 `state[0x26] ∈ {4,5}` 면 **타석 수 `+0x14` 증가(`0xa8b02`)와 기록 7 을 건너뛴다** → 견제로 끝난 판은 타석으로 안 센다. 웹 `pinchHitAi.ts:59-60` 의 "웹에는 견제가 없어 타석마다 세면 된다" 는 배선하는 순간 깨진다 | CORRECTIONS 2 · Q1 |
