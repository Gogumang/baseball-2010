 ...
 *   `vt10` 은 `0xa9f60` = `0..i 중 살아 있는 주자 수 > 주자 i 의 목표 루(+0x8c)` 다.
 *   곧 "제 뒤로 주자가 목표 루보다 많이 살아 있다" = **아직 그 루에서 밀려나지 못했다**.
 *   예: 1루 주자가 1루에 붙어 있으면 (산 주자 2 > 목표 1) 참, 2루로 뛰기 시작하면 (2 > 2) 거짓.
 ...
 *   이 모델의 `targetBase` 가 바로 이 칸이다 (`startBase` 는 원본 `+0x84` 쪽이다).
 ...
 * - **3b(`0xb3938`)는 옮기지 않았다.** 조건의 마지막이 `주자+0x8c != 주자+0x7c%4` 인데,
 *   `+0x8c` 는 "루에 자리 잡기"(`0xa0820` 이 `+0x7c`·`+0x80`·`+0x88`·`+0x8c`·`+0x90` 을 한꺼번에
 *   그 루로 적는다)에서만 갱신되는 칸이고, 이 모델에는 그 "자리 잡기" 에 해당하는 칸이 없다.
 *   억지로 `startBase` 에 맞춰 넣어 보니 **평범한 단타에서 주자가 다 죽는다**(1루 주자가 3루까지
 *   가는 원본 동작이 깨졌다) — 뜻을 모르는 채로 박느니 **빼 두고 이유를 적는다**. 3b 가 잡던 것은
 *   "이미 잡힌 플레이에서 도착한 주자를 그 루를 밟은 야수가 잡는" 리터치성 아웃으로 보인다(유력).
 * - **`주자+0x94`(판정끝)** 은 이 모델의 `settled`("목표점에 닿았다")와 뜻이 달라 쓰지 않는다.
 *   원본은 `+0x88`(요구 루)과 `+0x94` 를 `0xa95e8`/`0xa9620` 이 **함께 세우고**
 *   `0xaa0a8`/`0xa9648` 이 **함께 푼다** — 그래서 요구 루 하나로 본다 (**유력**).
 ...
 * `0..i` 중 살아 있는 주자 수가 주자 i 의 **목표 루**보다 많으면 참.
 */
export function isStillForced(runners: readonly RunnerState[], index: number): boolean {
  ...
  for (let k = 0; k <= index; k += 1) {
    ...
    if (k === index) return live > runner.targetBase
  }
  return true
}
 ...
 * `요구 루 > 목표 루` 인 주자의 `+0x94 = 0, +0x88 = −1` 로 포스를 푼다. 원본 순서 그대로다.
 */
export function releaseForcesAfterOut(runners: readonly RunnerState[]): readonly RunnerState[] {
  ...
  return runners.map((runner) => {
    ...
    if (runner.requiredBase <= runner.targetBase) return runner
