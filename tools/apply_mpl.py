...
OUTFIT_RULES: dict[str, str] = {
    ...
    # ── 수비 화면 바탕도 같은 잔디다 (S3-stadium-items.md 6-2 · 0x7885c 디스어셈으로 확정) ──
    #
    # 0x7885c 는 0x786c8 과 **명령어까지 같은 꼴**이다 (슬롯만 +0x2c → +0x14,
    # 문자열만 stadium/attack.* → stadium/defense.*). 뒤집기 `줄 = 1 − idx` 도 그대로다.
    # 리터럴 0x0007885d 는 온 파일에 **딱 하나**(풀 0x352cc) 뿐이고, 그 자리를 읽는 곳은
    # 수비 화면 준비 0x35218 하나다 — 0x3529c 가 `(SR[0x1ba] − 1, 1)` 을 넘긴다.
    # 그래서 "주/야간" 은 **틀렸고**, 벌 3개는 타석 잔디와 같은 세 가지다. 색이 이를 못 박는다:
    #   줄 0 짙은 녹색(#316939) · 줄 1 청록(#315d6b "항상 푸른색") · 줄 2 올리브(#636921)
    # 칸 3(특급천연잔디)은 줄 −1 = PZX 기본색(#4a8231, 가장 밝은 녹색).
    # ⚠️ 타석(0x352e8)과 달리 **대전 모드 8·9 갈래가 없다** — 수비 바탕은 시즌 홈경기에서만 바뀐다.
    'stadium/defense': '잔디 칸 v: 줄 2 − v (0 → 2 · 1 → 1 · 2 → 0 · 3 → 기본색) (0x7885c)',
}
